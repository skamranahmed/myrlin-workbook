...
- **test/search-dispatch.test.js made hermetic.** Three tests hit `/api/search` with the real Claude provider enabled, scanning the machine's actual transcript corpus; on multi-GB corpora the synchronous reads block the event loop past the time budget and hang the whole suite. Those tests now disable the real provider for their duration (finally-restored), exercising the dispatcher purely through stubs.
